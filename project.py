...
def show_board(user_id, board_type):
    ...
    if board_type == 'cookie' or board_type == 'free':
        posts = curr \
        .execute(f"SELECT * FROM {board_type}_board ") \
        .fetchall()
        conn.close()
        print(posts)
...
def show_post(user_id, board_type, post_num):
    ...
    try:
        if board_type == 'cookie' or board_type == 'free':
            post = curr \
            .execute(f"SELECT * FROM {board_type}_board WHERE number = {post_num}") \
            .fetchone()
            conn.close()
...
def add_post(user_id, board_type, Title=None, Content=None):
    ...
    if board_type == 'cookie' or board_type == 'free':
        curr.execute(f"INSERT INTO {board_type}_board (title, author, like, content) VALUES \
            ('{title}', '{user_id}', {like}, '{content}')")
        conn.commit()
...
def fix_post(user_id, board_type, post_num, Title = None, Content = None):
    ...
    content = request.form['Content']
    if board_type == 'cookie' or board_type == 'free':
        curr.execute(f"UPDATE {board_type}_board \
            SET title = '{title}', content = '{content}' WHERE number = {post_num}")
...
def delete(user_id, board_type, post_num, Data = None):
    ...
    if board_type == 'cookie' or  board_type == 'free':
        ...
        curr.execute(f"DELETE FROM {board_type}_board WHERE number = {post_num}")
